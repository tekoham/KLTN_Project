...
const Categories = forwardRef((props, ref) => {
    const { handleSetActiveCategories, category } = props
    ...
    const handleActiveButton = id => {
        ...
        _listButton?.forEach(btn => {
            btn?.id !== id || (btn?.id === id && btn?.active) ? (btn.active = false) : (btn.active = true)
        })
        id !== category ? handleSetActiveCategories(id) : handleSetActiveCategories(-1)
