...
const CreateNFT = () => {
  ...
  const [
    ...
    setCollectionFollowStepModalVisible,
  ] = useState(false)
  ...
  const handleUploadAvatar = async () => {
    const [result, err] = await nftService.uploadAvatar({ imgFile: avatar })
    console.log(result)
    console.log(err)
  ...
  useEffect(() => {
    setListOfCollections(listDummy)
    // const fetchListCollections = async () => {
    //   dispatch(globalLoading())
    //   const [dataCollection, err] =
    //     await collectionService.getListOfCollection()
    //   if (err) {
    //     message.error('Failed to fetch collections')
    //   } else if (dataCollection) {
    //     setListOfCollections(dataCollection)
    //   }
  ...
  const onFinish = async (values) => {
    ...
    const isOnSale = form.getFieldValue('onSaleStatus')
    const marketplaceType = form.getFieldValue('type')
    ...
    const collectionId = form.getFieldValue('collectionId')
    // dispatch(setUpdateProfileState(null))
    removeError()
    console.log(uploadValue)
    console.log(isOnSale)
    console.log(marketplaceType)
    console.log(minimumBidPriceValue)
    console.log(minimumBidStartDateValue)
    console.log(minimumBidExpireDateValue)
  ...
  return (
    <DefaultLayout>
          ...
          >
            ...
            <div className='select-nft-avatar'>
              <Label title='Upload File'>
                <Form.Item
                  ...
                  name='upload'
                >
                  <CustomDragger setuploadValue={setuploadValue} />
                </Form.Item>
              </Label>
            </div>
            {/* category NFT input */}
              ...
              <div className='choose-sale-type'>
                <div className='choose-sale-type_header'>
                  <div className='choose-sale-type_label'>
                    <span className='choose-sale-type_label__title'>
                      Put on marketplace
                    </span>
            ...
            {isError && (
              <div className='err-underline'>
                Please fix all error before create collectible
              </div>
            )}
          </Form>
        </div>
      ...
      {/* Date Time Picker */}
      {startDate === 0 && (
        <DateTimePicker onChange={onMinimumBidStartDateChange} visible={true} />
      )}
      {expirationDate === 0 && (
        <DateTimePicker
      ...
      {isCollectionFollowStepModalVisible && (
        <CollectionFollowStepModal
          visible={isCollectionFollowStepModalVisible}
        />
      )}
    </DefaultLayout>
  )
